...
%Creación de la matriz de grupos.
...
%inicialización matriz de centros
...
%Selección de centros: se seleccionan como centros iniciales algunos de los
...
%%%%% K-MEDIAS++ %%%%%%%%%%%%%%%%
% el primer centro aleatorio ya está elegido
    cont_centros = 1;
    ...
    for cent=2:parametros.numCentros
        for t=1:size(trainInputs,1)
            ...
            %determinar el centro más cercano de los que ya hay
...
for i=1:parametros.maxIter,
    ...
    for k=1:size(trainInputs, 1),
        %calcular cuál es el centro más cercano al ejemplo que se está
        %considerando
    ...
    if centrosAnt == red.centros
        fprintf('Finalizado en iteración %d\n', i)
        return
    end
